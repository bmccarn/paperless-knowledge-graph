...
_LABEL = re.compile(r"^(?:[A-Za-z][A-Za-z '\-]*?\s+)?(?:\((" + VALUE_UNITS + r")\)|(" + VALUE_UNITS + r"))$")
...
def _has_unit_annotation(label):
    return bool(re.search(r"(?<![A-Za-z'’])" + VALUE_UNITS, label)
...
def unit_names(text):
    return set(re.findall(r"(?<![A-Za-z'’/])" + VALUE_UNITS + r'(?![A-Za-z/])', text))


def prose_quantities(text):
    pairs = {(Decimal(amount.replace(',', '')), unit) for amount, unit in re.findall(
        r'(?<![\w.,])(' + NUMBER + r')\s*(' + VALUE_UNITS + r')(?![A-Za-z/])', text)}
