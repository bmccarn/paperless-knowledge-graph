...
| G2 — discriminate | Minimize the semantic reproducer while retaining its failure; preregister bounded, one-variable experiments against ranked hypotheses. | At least one candidate materially improves the reproduced failure without failing positive controls; all outcomes retained. | B2, addition-only recovery and source-record comparison remain failed. The approved [model comparison](reader-verifier-model-diagnostic.md) completed all 36 executions/261 calls; both independent grades FAIL. Fresh final retention is 54/66 versus 52/66. Occurrence reconciliation is frozen; source-scope and guard losses remain. The [source-grounded contract plan](source-grounded-interpretation-contract.md) selects an explicit source-scope hypothesis for concrete review, separately from the reviewed date-spacing repair. No semantic pass |
