...
4. Split synthetic source families into development and held-out sets before candidate work. Keep each semantic pair and all its wording/order variants in the same partition. A holdout is evaluated only after the candidate is frozen; inspecting a failed holdout turns it into development data and requires a new independently labelled holdout before the next release decision.
