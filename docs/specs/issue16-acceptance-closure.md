...
The original insurance question completed in the live browser in 169.1 seconds on b1eb57e, with 35/35 units audited and no token-limit or timeout error, but still withheld the answer. The ledger exposed formatting-only horizontal rules, a split numbered heading and a membership number detached at `No.` as unsupported claims. Preserve all factual heading/label text and exact candidate offsets while auditing it with the following sentence; omit only grammar-recognized thematic breaks. Keep numbered list prefixes and abbreviation-bound values together. Numeric validation may normalize ATX heading syntax and its ordered outline marker just as it already handles ordered lists; factual heading amounts and trailing headings remain audited. Add public-finalizer regressions including adversarial factual headings and invalidate the old source-audit cache.

The synthesis prompt also contradicted temporal acceptance by instructing the model to choose an active policy and infer supersession from overlapping terms. Replace those instructions with dated source observations and explicit-evidence requirements for cancellation/replacement; focus identification questions on identifying facts rather than unrelated coverage inventories. Keep the current-status acceptance gate unchanged. Verify the original question again through the actual browser and inspect source details and saved conversation state; supported documented facts with an explicit current-status limitation are acceptable, while false formatting rejection is not.
