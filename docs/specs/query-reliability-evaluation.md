...
| G2 — discriminate | Minimize the semantic reproducer while retaining its failure; preregister bounded, one-variable experiments against ranked hypotheses. | At least one candidate materially improves the reproduced failure without failing positive controls; all outcomes retained. | Completed B2 and addition-only recovery fail. Source-record comparison implemented, reviewed and admitted at 2e28074; approved native execution completed ten pairs and failed two on reader timeouts; both independent grades reject it, with no net retention gain and three unsupported final assertions. The reviewed [model comparison](reader-verifier-model-diagnostic.md) is next. No semantic pass |
